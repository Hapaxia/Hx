...
template <class positionT, class T>
T Piecewise<positionT, T>::getValue(positionT position)
{
	Node* lowerNode{ &m_nodes.front() };
	Node* higherNode{ &m_nodes.back() };
	for (auto& node : m_nodes)
	{
	...
	if (lowerNode == higherNode)
		...
		return hx::Tween::linear(lowerNode->value, higherNode->value, static_cast<double>(position - lowerNode->position) / (higherNode->position - lowerNode->position));
}
...
template <class positionT, class T>
unsigned int Piecewise<positionT, T>::getNodeCount()
{
	return m_nodes.size();
}
