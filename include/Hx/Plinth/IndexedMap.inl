...
template <class keyT, class T>
T IndexedMap<keyT, T>::get(const keyT& key)
{
	...
	throw "Indexed Map - key not found";
}

template <class keyT, class T>
T IndexedMap<keyT, T>::get(unsigned int index)
{
	if (indexIsValid(index))
		return m_elements[index].value;
	else
		throw "Indexed Map - index out of range";
}
...
template <class keyT, class T>
bool IndexedMap<keyT, T>::valid(const keyT& key)
{
	for (auto& element : m_elements)
	{
		if (element.key == key)
...
template <class keyT, class T>
bool IndexedMap<keyT, T>::valid(unsigned int index)
{
...
template <class keyT, class T>
keyT IndexedMap<keyT, T>::getKey(unsigned int index)
{
	if (indexIsValid(index))
...
template <class keyT, class T>
unsigned int IndexedMap<keyT, T>::getSize()
{
...
// PRIVATE
template <class keyT, class T>
bool IndexedMap<keyT, T>::indexIsValid(unsigned int index)
{
